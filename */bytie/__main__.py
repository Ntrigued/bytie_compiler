...
def main(argv: list[str] | None = None) -> None:
    ...
    if args.emit_bytecode:
        ast_path = Path(args.emit_bytecode)
        if not ast_path.exists():
            print(f"Error: file {ast_path} not found", file=sys.stderr)
            sys.exit(1)
        ...
        if name.endswith('.bytie.ast.json'):
            ...
            out_path = ast_path.with_name(out_name)
            ...
            if ast_path.suffix:
                out_path = ast_path.with_name(ast_path.stem + '.bytie.bc.json')
            else:
                out_path = ast_path.with_name(name + '.bytie.bc.json')
        with open(out_path, 'w', encoding='utf-8') as out:
