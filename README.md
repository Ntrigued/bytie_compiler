...
- `utilities/` – Utility scripts for working with the compiler, including:
  - `output_bytecode_code.py` – Extracts and formats bytecode from `.bytie.bc.json` files
...
The `utilities/` directory contains helpful scripts for working with the Bytie compiler:

### output_bytecode_code.py

This utility extracts and formats bytecode from `.bytie.bc.json` files, making it easier to read and analyze the generated bytecode.

**Usage:**
```sh
# Display bytecode on console
python utilities/output_bytecode_code.py <input_file.bc.json>

# Save bytecode to output file
python utilities/output_bytecode_code.py <input_file.bc.json> -o <output_file.txt>
```

**Example:**
```sh
# Process a bytecode file and display on console
python utilities/output_bytecode_code.py examples/program_1.bytie.bc.json

# Save formatted bytecode to a file
python utilities/output_bytecode_code.py examples/program_1.bytie.bc.json -o bytecode_output.txt
```
...
For more details, see `utilities/README.md`.
